...
export class ManageCardVendorsComponent extends BaseComponentModals<CardVendor> implements OnInit {
  ...
  viewObject(vendor: CardVendor): void {
    try {
      ...
      this.service.get<CardVendor>(vendor.id).subscribe(
        resp => {
          if (resp) {
            ...
            throw new Error(
              'Unable to get Card Vendor details from the server, please try again'
            );
          }
        },
        err => {
  ...
  processFormData(data) {
    try {
      ...
      if (this.modalOperation === 'create') {
        this.service.create<CardVendor>(this.selectedObject).subscribe(
          vendor => {
            this.selectedObject = vendor;
